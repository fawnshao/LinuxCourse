#!/bin/bash
clear
# cols[random_col]=17; coderain starts from line 17
awkscript='
{
...
while true; do
	...
	name=`cat bioinformatics2020.txt | shuf | head -1`
	wishes=`cat wishes.txt | shuf | head -1`
	echo -n -e "\033[44m $name \033[0m"
	# echo -e "\033[42;5m ******$wishes****** \033[0m"
	echo -e "\033[42m ******$wishes****** \033[0m"
	cat art20.txt | while read line; do
		echo | awk '{printf "\t\t\t\t\t"}'
	...
	time=0
	lines=$(tput lines) ## get how many lines in the screen
	cols=$(tput cols)  ## get how many columns in the screen
	while [ $time -lt 150 ]; do
		echo $lines $cols $(( $RANDOM % $cols )) $(( $RANDOM % 20 ))
		time=$(($time+1))
		sleep 0.03
	done | awk "$awkscript"
	echo -e "\033[0m"
